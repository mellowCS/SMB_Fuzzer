use crate::smb2::{
    handshake_helper::{
        fields::SecurityMode,
        negotiate_context::{
            CompressionCapabilities, ContextType, EncryptionCapabilities,
            NetnameNegotiateContextId, PreauthIntegrityCapabilities, RDMATransformCapabilities,
            TransportCapabilities, NegotiateContext,
        },
    },
    header, responses,
...
use super::convert_byte_array_to_int;
...
pub fn decode_negotiate_response_body(encoded_body: Vec<u8>) -> responses::negotiate::Negotiate {
    ...
    let buffer_end_index =
        64 + convert_byte_array_to_int(negotiate_response.security_buffer_length.clone()) as usize;
    negotiate_response.buffer = encoded_body[64..buffer_end_index].to_vec();

    match negotiate_response.dialect_revision.clone().unwrap() {
        DialectRevision::SMB311 => {
            ...
            let padding_end_index =
                convert_byte_array_to_int(negotiate_response.negotiate_context_offset.clone())
                    as usize
                    - HEADER_LENGTH;
            negotiate_response.padding = encoded_body[buffer_end_index..padding_end_index].to_vec();

            negotiate_response.negotiate_context_list = decode_negotiate_response_context(
                encoded_body,
                &negotiate_response,
                padding_end_index,
            );
...
pub fn decode_negotiate_response_context(
    ...
    let context_count =
        convert_byte_array_to_int(negotiate_response.negotiate_context_count.clone());
    ...
    for _ in 0..context_count {
        ...
        let context_size =
            (convert_byte_array_to_int(neg_context.data_length.clone()) + 8) as usize;
...
/// to the subroutines.
pub fn decode_generic_context(encoded_body: &Vec<u8>, start_index: usize) -> NegotiateContext {
...
pub fn decode_preauth_context(
    ...
    let salt_length = convert_byte_array_to_int(preauth_cap.salt_length.clone()) as usize;
...
pub fn decode_encryption_context(
    ...
    for counter in 0..convert_byte_array_to_int(encrypt_cap.cipher_count.clone()) {
...
pub fn decode_compression_context(
    ...
    for counter in 0..convert_byte_array_to_int(compress_cap.compression_algorithm_count.clone()) {
...
pub fn decode_netname_context(
    ...
    let name_length = convert_byte_array_to_int(name_length_in_bytes) as usize;
...
pub fn decode_rdma_transform_context(
    ...
    for counter in 0..convert_byte_array_to_int(rdma_cap.transform_count.clone()) {
...
mod tests {
    ...
    fn test_decode_response_header() {
        ...
        assert_eq!(
            64,
            convert_byte_array_to_int(decoded_header.generic.structure_size)
        );
