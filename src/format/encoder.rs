use super::{
    super::smb2::{
        handshake_helper::negotiate_context::{
...
pub fn serialize_negotiate_request(header: &SyncHeader, request_body: &Negotiate) -> Vec<u8> {
    let mut packet = serialize_sync_header(header);
    packet.append(&mut serialize_negotiate_request_body(request_body));
...
pub fn add_alignment_padding_if_necessary(data_length: Vec<u8>) -> Vec<u8> {
    vec![0; (8 - super::convert_byte_array_to_int(data_length) % 8) as usize]
}
...
mod tests {
    ...
    use crate::smb2::{
        handshake_helper::fields::Capabilities,
        handshake_helper::{fields, negotiate_context},
        header::Commands,
        requests,
    };
    ...
    fn test_serialize_negotiate_request_body() {
        ...
        negotiate_request.negotiate_context_list = vec![setup.negotiate_context_encrypt, setup.negotiate_context_compress];
        ...
        expected_byte_array.append(&mut b"\x62\x00\x00\x00\x02\x00\x00\x00\x11\x03\x00\x00".to_vec());
        expected_byte_array.append(&mut b"\x02\x00\x04\x00\x00\x00\x00\x00\x01\x00\x04\x00\x00\x00\x00\x00".to_vec());
        ...
        assert_eq!(expected_byte_array, serialize_negotiate_request_body(&negotiate_request));
    }
    ...
    fn test_serialize_negotiate_contexts() {
        ...
        assert_eq!(
            expected_byte_array,
            serialize_negotiate_contexts(vec![setup.negotiate_context_encrypt, setup.negotiate_context_compress])
        );
    }
